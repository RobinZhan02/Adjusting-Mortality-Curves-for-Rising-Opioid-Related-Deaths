...
p_expo <- hmd_5yr %>%
  ...
  theme_minimal(base_size = 12) +
  labs(
    title = "U.S. Population Pyramids (1995–2020)",
    x = "Age Group",
    y = "Exposure (person-years)"
  ) +
  theme(
...
p_expo_noleg <- p_expo + theme(legend.position = "none")
p_deaths_noleg <- p_deaths + theme(legend.position = "none")

legend <- cowplot::get_legend(
  p_expo + theme(legend.position = "bottom")
)

top_bottom <- cowplot::plot_grid(
  p_expo_noleg,
  p_deaths_noleg,
  ncol = 1,
  align = "v",
  rel_heights = c(1, 1)  # adjust if you want one row taller
)
